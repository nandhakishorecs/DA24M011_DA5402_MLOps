...
logging.basicConfig(filename='brownie_experiments.log', level=logging.INFO)
...
def run_experiment(version, seed):
    exp_name = f"brownie_{version}_seed{seed}"
    logger.info(f"Running experiment: {exp_name}")
    update_params(version, seed)
    os.system(f"dvc exp run --name {exp_name} --force")
...
if __name__ == "__main__":
    ...
    logger.info("Brownie experiments completed")